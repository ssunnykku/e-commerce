package kr.hhplus.be.server.common.seeder;
...
public class OrderSeeder implements CommandLineRunner {
    ...
    private static final int TOTAL_ORDER_PRODUCTS = 100_000;
    ...
    public void run(String... args) {
        ...
        int generatedOrderProducts = 0;

        while (generatedOrderProducts < TOTAL_ORDER_PRODUCTS) {
            ...
            for (Product p : selectedProducts) {
                ...
                ops.add(OrderProduct.of(
                        p.getId(), null, quantity, null, OrderStatus.ORDERED.getCode()
                ));
            }

            Order order = orderRepository.save(
                    Order.of(user.getUserId(), totalAmount, OrderStatus.ORDERED.getCode())
            );

            List<OrderProduct> realOps = ops.stream()
                    .map(op -> OrderProduct.of(
                            op.getProductId(), order.getId(), op.getQuantity(), order.getOrderDate(), op.getStatus()
                    ))
                    .collect(Collectors.toList());

            orderProductRepository.saveAll(realOps);
            generatedOrderProducts += realOps.size();

            if (generatedOrderProducts % 10_000 == 0) {
                System.out.println("진행 상황: " + generatedOrderProducts + "건 생성 완료");
            }
        }

        System.out.println("주문상품 10만건 더미 생성 완료");
    }
}