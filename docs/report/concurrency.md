...
- 사례 1: 동시에 포인트 충전 요청 (각 1000원) → 둘 다 기존 잔액을 기준으로 갱신 → 하나의 요청만 반영되어 총 1000원만 충전됨 (실제로는 2000원이 되어야 함)
- 사례 2: 두 요청이 거의 동시에 실행되어 잔액: 1000으로 조회됨 → 각각 500원 차감 → 실제로는 1000원이 차감되어야 하지만, 마지막 저장된 값은 500원만 차감된 상태
...
- 사례 2: 쿠폰 수량 3개, 3명 동시에 요청시 세명 모두 성공해야 함
- 사례 3: 한 사용자가 중복 요청을 보내 중복 발급되는 문제 발생
...
## 6. 동시성 테스트
- ✅ **상품 재고 차감**
  - 사례 1: 재고가 1개 남은 상품에 대해 3명이 동시에 주문 [OrderProcessorTest.java](https://github.com/ssunnykku/e-commerce/blob/STEP9/src/test/java/kr/hhplus/be/server/order/application/processor/OrderProcessorTest.java#L227)
- ✅ **유저 포인트 충전 및 사용**
  - 사례 1: 동시에 포인트 충전 요청 (각 1000원) [ChargeUseCaseTest.java](https://github.com/ssunnykku/e-commerce/blob/STEP9/src/test/java/kr/hhplus/be/server/user/application/useCase/integration/ChargeUseCaseTest.java#L81)
  - 사례 2: 잔액 1000원, 두 요청이 거의 동시에 각각 500원 차감
- ✅ **선착순 쿠폰 발급**
  - 사례 1: 쿠폰 수량이 3개인데 5명이 동시에 요청  [CreateCouponUseCaseTest](https://github.com/ssunnykku/e-commerce/blob/STEP9/src/test/java/kr/hhplus/be/server/coupon/application/useCase/Integration/CreateCouponUseCaseTest.java#L70)
  - 사례 2: 쿠폰 수량 3개, 3명 동시에 요청
  - 사례 3: 한 사용자가 중복 요청을 보내 중복 발급
  - 사례 4: 발급된 쿠폰을 동시에 사용하는 경우 (중복 사용)
