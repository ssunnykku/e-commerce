...
도메인별 **애플리케이션 서버/DB 분리** 시 발생하는 트랜잭션 한계를 명시하고, **대응 방안**(패턴/운영 전략/마이그레이션)을 제안한다.
...
- **주문 ID 생성**: 오케스트레이터가 Saga를 시작하기 전에 **Order 서비스**에 요청하여 **글로벌하게 유일한 주문 ID**를 먼저 생성하고 `PENDING` 상태로 저장한다. 이 ID를 이후의 모든 단계에서 **컨텍스트(context) 및 멱등성(idempotency) 키**로 사용한다.
...
#### 4.3 멱등성 & 중복 방지
- 서비스별 **IdempotencyKey**: `(orderId, step)` 유니크 인덱스
- 보상 트랜잭션도 **멱등**
